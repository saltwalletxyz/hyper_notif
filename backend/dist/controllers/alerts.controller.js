...
class AlertsController {
    async createAlert(req, res) {
        try {
            ...
            // Validate alert type
            if (!Object.values(client_1.AlertType).includes(type)) {
                res.status(400).json({ error: 'Invalid alert type' });
                return;
            }
            // Validate condition
            if (!Object.values(client_1.AlertCondition).includes(condition)) {
                res.status(400).json({ error: 'Invalid alert condition' });
                return;
            }
            // Create alert
            const alert = await database_service_1.db.alert.create({
                data: {
                    userId,
                    name,
                    type,
    ...
    async getAlerts(req, res) {
        try {
            const userId = req.user.id;
            const { page = '1', limit = '20', isActive, triggered, type, asset, } = req.query;
            const where = { userId };
            ...
            const [alerts, total] = await Promise.all([
                database_service_1.db.alert.findMany({
                    where,
                    skip,
                    take: limitNum,
                    orderBy: { createdAt: 'desc' },
                }),
                database_service_1.db.alert.count({ where }),
            ]);
            res.json({
                alerts,
                pagination: {
    ...
    async getAlert(req, res) {
        try {
            const userId = req.user.id;
            const { id } = req.params;
            const alert = await database_service_1.db.alert.findFirst({
                where: {
                    id,
                    userId,
                },
    ...
    async deleteAlert(req, res) {
        try {
            ...
            await database_service_1.db.alert.delete({
                where: { id },
            });
    ...
    async getAlertStats(req, res) {
        try {
            const userId = req.user.id;
            const [totalAlerts, activeAlerts, triggeredAlerts, alertsByType,] = await Promise.all([
                database_service_1.db.alert.count({ where: { userId } }),
                database_service_1.db.alert.count({ where: { userId, isActive: true } }),
                database_service_1.db.alert.count({ where: { userId, triggered: true } }),
                database_service_1.db.alert.groupBy({
                    by: ['type'],
                    where: { userId },
                    _count: true,
                }),
