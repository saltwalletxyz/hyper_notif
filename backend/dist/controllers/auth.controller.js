...
class AuthController {
    ...
    async connectWallet(req, res) {
        try {
            ...
            const existingUser = await database_service_1.db.user.findUnique({
            ...
            if (existingUser) {
                ...
                // Generate token
                const token = jsonwebtoken_1.default.sign({ userId: existingUser.id, walletAddress: normalizedAddress }, config_1.config.jwt.secret, { expiresIn: config_1.config.jwt.expiresIn });
                res.json({
                    user: {
                        id: existingUser.id,
    ...
    async registerWithWallet(req, res) {
        try {
            ...
            // Generate token
            const token = jsonwebtoken_1.default.sign({ userId: user.id, walletAddress: normalizedAddress }, config_1.config.jwt.secret, { expiresIn: config_1.config.jwt.expiresIn });
            res.status(201).json({
                user,
                token,
                isNewUser: true,
